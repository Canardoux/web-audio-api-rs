...
pub trait AudioScheduledSourceNode: AudioNode {
    ...
    /// [`AudioBufferSourceNode`](crate::api::node::AudioBufferSourceNode), the event is also dispatched
...
#[cfg(test)]
mod tests {
    use crate::api::context::{AudioContextRegistration, BaseAudioContext, OfflineAudioContext};
    use crate::api::node::{AudioNode, AudioScheduledSourceNode, ChannelConfig};

    use std::sync::atomic::{AtomicBool, Ordering};
    use std::sync::Arc;

    enum ConcreteAudioScheduledSourceNode {
        Buffer(crate::api::node::AudioBufferSourceNode),
        Constant(crate::api::node::ConstantSourceNode),
        Oscillator(crate::api::node::OscillatorNode),
    }
