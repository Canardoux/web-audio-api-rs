...
use super::api::{AudioBackendManager, RenderThreadInit};

use crate::api::buffer::AudioBuffer;
use crate::api::context::AudioContextOptions;
use crate::api::io::microphone::MicrophoneRender;
use crate::api::media_devices::{MediaDeviceInfo, MediaDeviceInfoKind};
use crate::api::render::RenderThread;
use crate::api::{MAX_CHANNELS, RENDER_QUANTUM_SIZE};
...
impl AudioBackendManager for CubebBackend {
        ...
        Self: Sized,
    {
        ...
        for (device, kind) in input_devices.chain(output_devices) {
            ...
            loop {
                let device_id = crate::api::media_devices::DeviceId::as_string(
                    kind,
                    "cubeb".to_string(),
