use crate::api::context::ConcreteBaseAudioContext;
use crate::api::context::{AudioContextState, AudioNodeId};
use crate::api::buffer::AudioBuffer;
use crate::api::capacity::AudioRenderCapacityEvent;
use std::any::Any;
...
#[frb(opaque)]
pub struct Event {
    pub type_: RustAutoOpaque<&'static str>,
}
...
pub struct ErrorEvent {
    ...
    pub error: RustAutoOpaque<Box<dyn Any + Send>>,
    /// Inherits from this base Event
    pub event: RustAutoOpaque<Event>,
}
...
pub struct AudioProcessingEvent {
    /// The input buffer
    pub input_buffer: RustAutoOpaque<AudioBuffer>,
    /// The output buffer
    pub output_buffer: RustAutoOpaque<AudioBuffer>,
...
impl Drop for AudioProcessingEvent {
    fn drop(&mut self) {
        if let Some((context, id)) = self.registration.take() {
            let wrapped = crate::api::message::ControlMessage::NodeMessage {
                id,
...
pub struct OfflineAudioCompletionEvent {
    ...
    pub rendered_buffer: RustAutoOpaque<AudioBuffer>,
    /// Inherits from this base Event
    pub event: RustAutoOpaque<Event>,
}
...
pub(crate) enum EventPayload {
    ...
    Diagnostics(Vec<u8>),
    Message(RustAutoOpaque<Box<dyn Any + Send + 'static>>),
...
impl EventDispatch {
    ...
    pub fn message(id: AudioNodeId, value: RustAutoOpaque<Box<dyn Any + Send + 'static>>) -> Self {
        EventDispatch {
