...
use crate::api::buffer::{AudioBuffer, AudioBufferOptions};
use crate::api::context::{
    AudioContextRegistration, AudioContextState, AudioParamId, ConcreteBaseAudioContext,
    DESTINATION_NODE_ID,
};
use crate::api::decoding::MediaDecoder;
use crate::api::events::{Event, EventHandler, EventType};
use crate::api::node::{AudioNode, AudioNodeOptions};
use crate::api::param::AudioParamDescriptor;
use crate::api::periodic_wave::{PeriodicWave, PeriodicWaveOptions};
use crate::api::{node, AudioListener};
...
pub trait BaseAudioContext {
    ...
    ) -> (crate::api::param::AudioParam, AudioParamId) {
        let param = self.base().register(move |registration| {
            let (node, proc) = crate::api::param::audio_param_pair(opts, registration);
    ...
    fn mock_registration(&self) -> AudioContextRegistration {
        AudioContextRegistration {
            id: crate::api::context::AudioNodeId(0),
            context: self.base().clone(),
        }
    }
}
