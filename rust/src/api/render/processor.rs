...
use crate::api::context::{AudioNodeId, AudioParamId};
use crate::api::events::{AudioProcessingEvent, ErrorEvent, EventDispatch};
use crate::api::{AudioBuffer, Event};
use crate::api::api::RENDER_QUANTUM_SIZE;
...
/// [`AudioNode`](crate::api::node::AudioNode) (the user facing object that lives in the control
/// thread). See [`ConcreteBaseAudioContext::register`](crate::api::context::ConcreteBaseAudioContext::register).
///
...
pub trait AudioProcessor: Send {
    ...
    /// via
    /// [`AudioContextRegistration::post_message`](crate::api::context::AudioContextRegistration::post_message).
...
/// Accessor for current [`crate::api::param::AudioParam`] values
///
...
impl<'a> AudioParamValues<'a> {
    ...
    //pub fn get(&self, index: &AudioParamId) -> impl Deref<Target = [f32]> + '_ { // [LARPOUX]
    pub fn get(&self, index: &AudioParamId) -> f32 { // [LARPOUX]
        DerefAudioRenderQuantumChannel(self.nodes.get_unchecked(index.into()).borrow())
    }

    pub(crate) fn listener_params(&self) -> f32 { // [LARPOUX]
    //pub(crate) fn listener_params(&self) -> [impl Deref<Target = [f32]> + '_; 9] { // [LARPOUX]
        crate::api::context::LISTENER_AUDIO_PARAM_IDS.map(|p| self.get(&p))
    }
}
