use crate::api::context::{AudioNodeId, DESTINATION_NODE_ID};
use crate::api::render::graph::Node;
...
impl NodeCollection {
    ...
    #[inline(always)]
    pub fn keys(&self) ->  dyn Iterator<Item = AudioNodeId> { // [LARPOUX]
    //pub fn keys(&self) -> impl Iterator<Item = AudioNodeId> + '_ { // [LARPOUX]
        self.nodes
            .iter()
            .enumerate()
...
/* [LARPOUX]
    #[inline(always)]
    pub fn values_mut(&mut self) -> impl Iterator<Item = &mut RefCell<Node>> { // [LARPOUX]
        self.nodes.iter_mut().filter_map(Option::as_mut)
    }
*/
    #[inline(always)]
    pub fn iter_mut(&mut self) -> dyn Iterator<Item = (AudioNodeId, &mut RefCell<Node>)> {
