...
pub use crate::api::render::AudioWorkletGlobalScope;

use crate::api::context::context::{AudioContextRegistration, AudioParamId};
use crate::api::context::BaseAudioContext;
use crate::api::node::{AudioNode, AudioNodeOptions, ChannelConfig};
use crate::api::param::{AudioParam, AudioParamDescriptor};
use crate::api::render::{AudioProcessor, AudioRenderQuantum, AudioParamValues};
use crate::api::MessagePort;
use crate::api::api::MAX_CHANNELS;
...
pub struct AudioParamValues<'a> {
    values: crate::api::render::AudioParamValues<'a>,
...
impl<'a> AudioParamValues<'a> {
    ...
    pub fn get(&'a self, name: &str) -> Vec<f32> { // [LARPOUX]
    // pub fn get(&'a self, name: &str) -> impl Deref<Target = [f32]> + 'a { // [LARPOUX]
        let id = self.map.get(name).unwrap();
        self.values.get(id)
    }

    pub fn keys(&self) -> i32 {//impl Iterator<Item = &str> { // [LARPOUX]
...
pub struct AudioWorkletNodeOptions<C> {
    ...
    pub RustAutoOpaque<parameter_data: HashMap<String, f64>>,
    ...
    pub RustAutoOpaque<processor_options: C>,
    /// Channel config options
    pub RustAutoOpaque<audio_node_options: AudioNodeOptions>,
}
...
impl AudioWorkletNode {
    ...
    pub fn new<P: AudioWorkletProcessor + 'static>(
        context: &i32, // [LARPOUX]
        //context: &impl BaseAudioContext, // [LARPOUX]
        ...
        let output_channel_count = if output_channel_count.is_empty() {
            ...
            output_channel_count
                .iter()
                .copied()
                .for_each(crate::api::api::assert_valid_number_of_channels);
            assert_eq!(
...
impl<P: AudioWorkletProcessor> AudioProcessor for AudioWorkletRenderer<P> {
    fn process(
        ...
        params: crate::api::render::AudioParamValues<'_>,
...
mod tests {
    use super::*;
    use crate::api::context::OfflineAudioContext;
