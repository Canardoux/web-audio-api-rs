...
use std::error::Error;
use std::iter::{Iterate, iterate, Iterator};
...
pub trait AudioBufferIter: Iterator<Item = FallibleBuffer> + Send + 'static {}

impl<M: Iterator<Item = FallibleBuffer> + Send + 'static> AudioBufferIter for M {}

pub type FallibleBuffer = Result<crate::api::buffer::AudioBuffer, Box<dyn Error + Send + Sync>>;
