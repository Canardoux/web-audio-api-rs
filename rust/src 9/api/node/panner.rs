...
use crate::api::context::{AudioContextRegistration, AudioParamId, BaseAudioContext};
...
impl PannerNode {
    ...
    pub fn new<C: BaseAudioContext>(context: &C, options: PannerOptions) -> Self {
        let mut node = context.base().register(|registration| {
            use crate::api::spatial::PARAM_OPTS;
    ...
    fn process(
        ...
        let mut a_rate_params = source_position_x
            ...
            .map(|tuple| {
                ...
                let (azimuth, elevation) = crate::api::spatial::azimuth_and_elevation(
                    source_position,
                    listener_position,
                    listener_forward,
                    listener_up,
...
impl PannerRenderer {
    fn cone_gain(
        ...
        if abs_inner_angle >= 180. && abs_outer_angle >= 180. {
            ...
            let abs_angle =
                crate::api::spatial::angle(source_position, source_orientation, listener_position);
    ...
    fn dist_gain(&self, source_position: [f32; 3], listener_position: [f32; 3]) -> f32 {
        ...
        let distance = crate::api::spatial::distance(source_position, listener_position) as f64;
...
mod tests {
    ...
    use crate::api::context::{BaseAudioContext, OfflineAudioContext};
    use crate::api::node::{AudioBufferSourceNode, AudioBufferSourceOptions, AudioScheduledSourceNode};
    use crate::api::AudioBuffer;
